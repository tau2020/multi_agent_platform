# agents/agent_backend_developer.py
...
class BackEndDeveloperAgent:
    def __init__(self, agent_id, input_queue, output_queue, model_type='openai'):
        ...
        self.specialization = 'Node.js'
        self.input_queue = input_queue
        self.output_queue = output_queue
        self.model_type = model_type
        self.llm_model = self.load_model()
        self.logger = logging.getLogger(f'BackEndDeveloperAgent-{self.agent_id}')
    ...
    async def work_on_task(self, task):
        ...
        api_code = await self.generate_api(task['description'])
        await self.submit_output(task['id'], api_code)
        self.logger.info(f"Completed task: {task['id']}")

    async def generate_api(self, description):
        prompt = (
            f"Write clean, well-documented Node.js (Express.js) code to accomplish the following task:\n\n"
            f"{description}\n\n"
            "Ensure that the code follows best practices, includes necessary imports, "
